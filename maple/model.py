...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_IVLP(width, heads, self.attn_mask, True,
                                                                         text_layer, i,
                                                                         design_details) if prompts_needed > i
                                             else ResidualAttentionBlock_IVLP(width, heads, self.attn_mask, False,
                                                                              text_layer, i, design_details)
                                             for i in range(layers)])
        elif current_trainer == 'MaPLe':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, self.attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
            # Corresponds to default CoOp or CoCoOp
            assert current_trainer == 'CoOp' or current_trainer == 'CoCoOp'
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, self.attn_mask) for _ in range(layers)])
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, text_attr=None):
        ...
        # print("Shape of x before adding positional embedding:", x.shape)
        # print("Shape of self.positional_embedding:", self.positional_embedding.shape)
        # print(dd)
        ...
        if text_attr != None:
            text_attr = text_attr.expand(x.shape[0], -1, -1).half()
            x = torch.cat([x, text_attr], dim=1)
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, text_attr=None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        if self.VPT_shallow:
            ...
            if text_attr == None:
                ...
                x = torch.cat([x, visual_ctx, text_attr], dim=1)
        else:
